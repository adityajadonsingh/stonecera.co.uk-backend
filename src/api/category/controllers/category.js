...
const { transformCategory } = require("../services/category-transformer");

const {
  getCategoryBySlug,
  getCategoryVariants,
} = require("../services/category-query");
...
module.exports = createCoreController(
  ...
  ({ strapi }) => ({
    ...
    async customDetail(ctx) {
      ...
      const category = await getCategoryBySlug(strapi, slug);

      if (!category) {
        return ctx.notFound("Category not found");
      }

      const variants = await getCategoryVariants(strapi, category.id);
